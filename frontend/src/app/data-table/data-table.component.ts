...
export class DataTableComponent {
  ...
  isLosersMoviesView: boolean = false;
  ...
  saveNewMovie() {
    this.isLosersMoviesView = false;
    this.isLosersDirectorsView = false;
  ...
  getLosersDirectors() {
    this.isLosersMoviesView = false;
    this.isLosersDirectorsView = true;
    this.fetchTableData();
  }

  updateMovie(event: any) {
    if (this.isLosersDirectorsView) {
      return;
    }
    this.isTableLoading = true;
  ...
  private fetchTableData(httpParams?: HttpParams) {
    this.isTableLoading = true;
    if (this.isLosersMoviesView) {
      this.movieService.getLosersMovies(httpParams).subscribe({
        next: page => {
    ...
    } else if (this.isLosersDirectorsView) {
      this.movieService.getLosersDirectors(httpParams).subscribe({
        next: page => {
          this.movies = [];
          page.result.forEach(person => {
            this.movies.push({
              screenwriter: person
            })
          });
        ...
        error: err => {
          this.messageService.add({
            severity: 'error',
            summary: 'Error finding losers directors',
            detail: err.error.message ?? err.message
          });
        }
      }).add(() => {
